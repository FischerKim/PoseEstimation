...
#raspberry pi camera
#camSet ='nvarguscamerasrc sensor-id=0 wbmode=3 tnr-mode=2 ! video/x-raw(memory:NVMM), width=3264, height=2464, framerate=21/1,format=NV12 ! nvvidconv flip-method='+str(flip)+' ! video/x-raw, width='+str(width)+', height='+str(height)+', format=BGRx ! videoconvert ! video/x-raw, format=BGR ! videobalance contrast=1.5 brightness=-.15 saturation 1.2 ! appsink' 

#webcam
camSet ='v4l2src device=/dev/video0 ! video/x-raw,width='+str(width)+',height='+str(height)+',frame=24/1 ! videoconvert ! appsink'
#camSet ='v4l2src device=/dev/video0 ! video/x-raw,width='+str(width)+',height='+str(height)+',frame=24/1 ! videoconvert ! video/x-raw, format=BGR ! videobalance contrast=1.5 brightness=-.15 saturation 1.2 ! appsink' 
...
save_name = f"{camSet.split('/')[-1].split('.')[0]}"
